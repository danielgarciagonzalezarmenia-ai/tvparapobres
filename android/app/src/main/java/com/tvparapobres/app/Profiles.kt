...
object Profiles {
    ...
    data class Profile(val id: String, val name: String, val color: String, val pin: String)
    ...
    fun profiles(): List<Profile> {
        ...
        val list = loadRaw().mapNotNull { j ->
            ...
            Profile(id, name, j.optString("color", DEFAULT_COLOR), j.optString("pin", ""))
        }
        cached = list
        return list
    }
    ...
    fun add(name: String, color: String, pin: String): String {
        val p = Profile(newId(), name, color, pin)
        save(profiles() + p)
        return p.id
    }
